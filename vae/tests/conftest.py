...
@pytest.fixture
def make_runtime():
    """Factory of a Runtime over fakes; msd's real file store serves `workspace`."""
    def factory(workspace: Path = None, production_runner=None, source_repo=None, config_repo=None) -> Runtime:
        ...
        return Runtime(
            ...
            model_setup_data_store=msd.model_setup_data_store(workspace or Path("/nonexistent-workspace")),
        )
    return factory
...
@pytest.fixture
def produce():
    """Writes a produced file where msd's production would have left it and returns its path."""

"""The Celery production runner with the task and AsyncResult mocked (SRS DSM-VAE req 6)."""
...
def _status(state, result=None) -> ProductionStatus:
    with mock.patch("vae.adapters.celery_production_runner.AsyncResult", return_value=_AsyncResult(state, result)):
        return CeleryProductionRunner().status(seed.RUN_1)


def test_start_submits_the_selection_and_sources_as_json_payloads():
    with mock.patch("vae.adapters.celery_production_runner.produce_model_setup_data") as task:
...
def test_cancel_revokes_with_terminate_and_reports_the_status():
    with mock.patch("vae.adapters.celery_production_runner.AsyncResult") as async_result:

"""Loads vae.ini: the data source addresses, the API, the worker and the Redis
container settings (SRS DSM-VAE req 4, 7)."""
...
VAE_INI = Path(__file__).resolve().parent / "vae.ini"
...
def load(path: Path = VAE_INI) -> Config:
...
@lru_cache
def config() -> Config:
    """The shipped vae.ini, loaded once."""
    return load()

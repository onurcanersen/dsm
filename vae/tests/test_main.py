"""The dsm command with the Docker SDK and the child processes mocked (SRS DSM-VAE req 6)."""
...
from vae import __main__ as dsm
from vae.config import RedisConfig
...
def test_starts_an_existing_container_then_both_children_and_stops_everything_on_ctrl_c(world):
    ...
    assert [child.argv for child in world["children"]] == [
        [sys.executable, "-m", "vae.worker"],
        [sys.executable, "-m", "vae.api"],
    ]
...
def test_concurrency_is_passed_to_the_worker(world):
    ...
    assert world["children"][0].argv == [sys.executable, "-m", "vae.worker", "--concurrency=4"]

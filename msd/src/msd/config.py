"""Loads msd.ini: the workspace and system repo name, the mandatory file rules,
the source parser settings and the build switch (SRS DSM-MSD req 3, 13, 15, 19)."""
...
MSD_INI = Path(__file__).resolve().parent / "msd.ini"
...
class WorkspaceConfig:
    ...
    def root(self) -> Path:
        """The workspace root, relative to the msd package directory unless absolute."""
        path = Path(self.path)
...
def load(path: Path = MSD_INI) -> Config:

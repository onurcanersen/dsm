...
produced files (req 5) and the production runs (req 6, 8, 50) (SRS DSM-VAE)."""
...
from msd import ConfigManagementAccessError, SourceRepoAccessError, SourceRepoAuthError, SourceType
...
@pytest.mark.parametrize("method, path, connect, message", [
    ...
    ("post", "/api/msd/run", "config_db", "source code repo connection required"),
])
...
def test_run_starts_a_production_with_the_sessions_sources_and_user(make_runtime, make_signed_in_client, login):
    ...
    response = client.post("/api/msd/run", json=dict(seed.SELECTION, candidate=seed.CANDIDATE))
...
def test_run_without_a_candidate_submits_none(make_runtime, make_signed_in_client):
    ...
    assert client.post("/api/msd/run", json=seed.SELECTION).status_code == 202
...
def test_run_refuses_a_malformed_candidate(signed_in_client, candidate, message):
    response = signed_in_client.post("/api/msd/run", json=dict(seed.SELECTION, candidate=candidate))
...
def test_run_validates_the_selection(signed_in_client):
    assert "missing field" in signed_in_client.post("/api/msd/run", json={}).get_json()["error"]
    assert signed_in_client.post("/api/msd/run", data=b"not json", content_type="application/json").status_code == 400
...
def test_runner_failure_to_start_maps_to_502(make_runtime, make_signed_in_client):
    ...
    response = make_signed_in_client(make_runtime(production_runner=BrokenRunner())).post("/api/msd/run", json=seed.SELECTION)
...
def test_task_state_serves_the_status_and_the_lines_after_the_cursor(make_runtime, make_signed_in_client):
    ...
    client = make_signed_in_client(runtime)
    task_id = client.post("/api/msd/run", json=seed.SELECTION).get_json()["task_id"]
    for line in ("line one", "line two", "line three"):
        runtime.production_log.append(task_id, line)

    body = client.get(f"/api/msd/tasks/{task_id}").get_json()
    assert body["state"] == "SUCCESS"
    assert body["result"] == {"selection": seed.SELECTION}
    assert body["lines"] == ["line one", "line two", "line three"]

    assert client.get(f"/api/msd/tasks/{task_id}?after=1").get_json()["lines"] == ["line three"]
    assert client.get(f"/api/msd/tasks/{task_id}?after=not-a-number").get_json()["lines"] == ["line one", "line two", "line three"]
...
def test_task_state_reports_progress_while_the_state_holds(make_runtime, make_signed_in_client):
    ...
    polls = [client.get(f"/api/msd/tasks/{task_id}").get_json() for _ in range(4)]
...
def test_cancel_revokes_a_queued_run_and_reports_a_finished_one(signed_in_client):
    task_id = signed_in_client.post("/api/msd/run", json=seed.SELECTION).get_json()["task_id"]

    assert signed_in_client.post("/api/msd/tasks/does-not-exist/cancel").get_json() == {"task_id": "does-not-exist", "state": "REVOKED"}
    assert signed_in_client.post(f"/api/msd/tasks/{task_id}/cancel").get_json()["state"] == "SUCCESS"
...
def test_cancel_and_state_failures_map_to_502(make_runtime, make_signed_in_client):
    ...
    assert "broker down" in client.post("/api/msd/tasks/x/cancel").get_json()["error"]
    assert "backend down" in client.get("/api/msd/tasks/x").get_json()["error"]


@pytest.mark.parametrize("method, path", [("post", "/api/msd/tasks/x/cancel"), ("get", "/api/msd/tasks/x")])

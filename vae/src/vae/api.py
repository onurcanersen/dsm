"""VAE's Flask API: sign-in, data source connections, the selection, the
produced Model Setup Data files and the production runs
(SRS DSM-VAE req 3-8, 50).
...
Endpoints:
  ...
  GET    /api/projects/.../versions/<version_id>/msd-files                                               [login + config_mgmt_db]
  GET    /api/projects/.../versions/<version_id>/msd-files/<run_id>/model                                [login + config_mgmt_db]
  GET    /api/projects/.../versions/<version_id>/msd-files/<run_id>/download                             [login + config_mgmt_db]
  GET    /api/units/<unit_name>/versions                                                                 [login + source_code_repo]
  POST   /api/msd/run                                  selection fields + "candidate"?                   [login + both]
  GET    /api/msd/tasks/<task_id>?after=<line>                                                           [login]
  POST   /api/msd/tasks/<task_id>/cancel                                                                 [login]

Run with: python -m vae.api (the dsm command starts it together with the worker).
"""
...
from vae import Runtime, runtime as load_runtime
...
def _candidate(body: dict):
    """The candidate unit version in the body (SRS DSM-MSD req 11): None when absent, a 400 response when malformed."""
...
def create_app(runtime: Runtime | None = None) -> Flask:
    """The Flask application over a runtime; the vae.ini runtime when none is given."""
    ...
    def api_units(project_id, platform_id, version_id):
        """The Software Unit Version Inventory of a selection (SRS DSM-MSD req 10)."""
    ...
    def api_unit_versions(unit_name):
        """The versions the source code repository publishes for a unit (SRS DSM-MSD req 11)."""
    ...
    @app.route("/api/projects/<project_id>/platforms/<platform_id>/versions/<version_id>/msd-files/<run_id>/model")
    @login_required
    @config_db_required
    ...
    @app.route("/api/projects/<project_id>/platforms/<platform_id>/versions/<version_id>/msd-files/<run_id>/download")
    @login_required
    @config_db_required
    ...
    def api_msd_run():
        ...
        try:
            ...
            logger.warning("msd/run: submission failed: %s", exc)
            return _error(str(exc), 502)

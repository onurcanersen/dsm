"""vae.ini loading and the runtime's check for the two data source sections (SRS DSM-VAE req 4, 7)."""

import pytest

import vae
from fakes import seed
from msd import SourceType
from vae.config import VAE_INI, Config, load


def test_shipped_vae_ini_is_loaded():
    config = load(VAE_INI)
...
def test_runtime_refuses_an_ini_without_both_data_sources(monkeypatch):
    monkeypatch.setattr(vae, "config", lambda: Config(data_sources=[seed.DEFAULTS[SourceType.CONFIG_MGMT_DB]]))
